...
import com.c414.dload.mall_backend.entity.UmsResource;
...
import com.c414.dload.mall_backend.service.IUmsAdminCacheService;
...
public class UmsAdminCacheServiceImpl implements IUmsAdminCacheService {
    ...
    public void delResourceListByRoleIds(List<Long> roleIds) {
        LambdaQueryWrapper<UmsAdminRoleRelation> wrapper = new LambdaQueryWrapper<>();
        wrapper.in(UmsAdminRoleRelation::getRoleId, roleIds);
        List<UmsAdminRoleRelation> relationList = adminRoleRelationMapper.selectList(wrapper);
        if (CollUtil.isNotEmpty(relationList)) {
            String keyPrefix = REDIS_DATABASE + ":" + REDIS_KEY_RESOURCE_LIST + ":";
            List<String> keys = relationList.stream().map(relation -> keyPrefix + relation.getAdminId()).collect(Collectors.toList());
